...
from app.domain.jobs.taxonomies import CERTIFICATIONS
from app.parsers.jobs.phrase_matcher import compile_phrase_patterns, find_phrase_matches

CERTIFICATION_PATTERNS = compile_phrase_patterns(tuple(CERTIFICATIONS))
CERTIFICATION_VALUES = MappingProxyType(
    {phrase: (value, "certification") for phrase, value in CERTIFICATIONS.items()}
)
...
class CertificationParser:
    def parse(self, units: tuple[EvidenceUnit, ...]) -> ParserFragment:
        ...
        values: list[str] = []
        for unit in units:
            for match in find_phrase_matches(
                unit.text, CERTIFICATION_PATTERNS, CERTIFICATION_VALUES
            ):
                requirements.append(
                    ParsedRequirement(
